package com.project.finalandproject.openapi.search;
...
public class Searcher {
	...
	public static final String DAUM_MAPS_LOCAL_CATEGORY_SEARCH_API_FORMAT = "https://apis.daum.net/local/v1/search/category.json?code=%s&location=%f,%f&page=%d&apikey=%s";
	/** category codes
	MT1 대형마트
	CS2 편의점
	PS3 어린이집, 유치원
	...
	public void searchKeyword(Context applicationContext, String query, double latitude, double longitude, int radius, int page, String apikey, OnFinishSearchListener onFinishSearchListener) {
		...
		String url = buildKeywordSearchApiUrlString(query, latitude, longitude, page, apikey);
		Log.i("url",url);
	...
	public void searchCategory(Context applicationContext, String categoryCode, double latitude, double longitude, int radius, int page, String apikey, OnFinishSearchListener onFinishSearchListener) {
		...
		String url = buildCategorySearchApiUrlString(categoryCode, latitude, longitude, page, apikey);
		searchTask = new SearchTask();
		Log.i("url2",url);
		searchTask.execute(url);
	}
    
	private String buildKeywordSearchApiUrlString(String query, double latitude, double longitude, int page, String apikey) {
    	String encodedQuery = "";
		try {
			encodedQuery = URLEncoder.encode(query, "utf-8");
			Log.i("Encode",encodedQuery);
			Log.i("Encode",query);
	...
	private String buildCategorySearchApiUrlString(String categoryCode, double latitude, double longitude, int page, String apikey) {
		return String.format(DAUM_MAPS_LOCAL_CATEGORY_SEARCH_API_FORMAT, categoryCode, latitude, longitude, page, apikey);
	}
